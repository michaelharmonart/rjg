...
class MetaToe(rModule.RigModule, rFk.Fk, rIk.Ik):
    def __init__(
        self,
        side=None,
        part=None,
    ...
    def add_bendy_twist(self, ctrl_scale=None, mirror=True, global_scale_attr=None):
        """
        Adds a bend-twist chain rig for the toe.
        This creates Start, Mid, and End bendy controls per segment in the toe chain,
    ...
    def add_plugs(self):
        ...
        if not self.par_ctrl:
            if self.part != 'Backtoe':

                driver_list = ['foot_' + self.side + '_02_switch_JNT']
                rAttr.Attribute(node=self.part_grp, type='plug', value=['foot_' + self.side + '_02_JNT'], name='skeletonPlugs', children_name=[self.bind_joints[0]])
            elif self.part == 'Backtoe':

                driver_list = ['foot_' + self.side + '_01_switch_JNT']
                rAttr.Attribute(node=self.part_grp, type='plug', value=['foot_' + self.side + '_01_JNT'], name='skeletonPlugs', children_name=[self.bind_joints[0]])
        else:
        ...
        if self.part == 'Thumbtoe':
            ...
            if self.expression_control:
                if mc.objExists(f'foot_{self.side}_express_CTRL'):
                    falloff = None
                    if self.part == 'Indextoe':
                        mult1value = 1
                        mult2value = 0
                    elif self.part == 'Middletoe':
                        mult1value = 0
                        mult2value = 0
                        falloff = 'inner'
                    elif self.part == 'Ringtoe':
                        mult1value = 0
                        mult2value = 0 
                        falloff = 'outer'
                    elif self.part == 'Pinkytoe':
                        mult1value = 0
                        mult2value = 1
                    else:
                        mult1value = 0
        ...
        if self.curl:
            ...
            if self.expression_control:
                mc.addAttr(f'foot_{self.side}_express_CTRL', longName=f'{self.base_name}curl', proxy=f'{self.curl_ctrl.ctrl}.rotate{self.curlaxis}')
