...
### Micheal's UV PIN Ribbon Code
    ...
    def generate_ribbon(
        ...
        surface_shape = mc.listRelatives(nurbs_surface_name, shapes=True)[0]
        if mc.nodeType(surface_shape) == "nurbsSurface":
            if swap_uv:
                ribbon_length = mc.getAttr(str(surface_shape)+".minMaxRangeV")[0][1]
                ribbon_width = mc.getAttr(str(surface_shape)+".minMaxRangeU")[0][1]
            else:
                ribbon_length = mc.getAttr(str(surface_shape)+".minMaxRangeU")[0][1]
                ribbon_width = mc.getAttr(str(surface_shape)+".minMaxRangeV")[0][1]
            if not number_of_controls:
                number_of_controls = int(ribbon_length)
                if half_controls:
                    number_of_controls = int(number_of_controls/2)

            ribbon_object: str = mc.duplicate(nurbs_surface_name, name=nurbs_surface_name+"_ribbon")[0]
            ribbon_group = mc.group(ribbon_object, name=ribbon_object+"_GRP")
            ...
            if control_joints:
                ...
                if cyclic:
                    for i in range(number_of_controls):
                        ...
                        if swap_uv:
                            position = mc.pointOnSurface(ribbon_object, position=True, parameterU=v, parameterV=u)
                            print(u,v)
                        else:
                            position = mc.pointOnSurface(ribbon_object, position=True, parameterU=u, parameterV=v)
                            print(u,v)
                        mc.select(ribbon_group, replace=True)
                        joint_name = mc.joint(position=position, radius=1, name=str(ribbon_object)+str(i+1)+"_ControlJoint")
                else:
            ...
            if cyclic:
                for i in range(number_of_joints):
                    ...
                    if swap_uv:
                        ...
                        position = mc.pointOnSurface(ribbon_object, position=True, parameterU=u, parameterV=v)
                    joint_name = mc.joint(position=position, radius=0.5, name=str(ribbon_object)+"_point"+str(i+1)+"_DEF")
                    mc.select(ribbon_object+".uv"+"["+str((i/(number_of_joints-2))*2)+"]"+"[0.5]", replace=True)
                    mc.select(joint_name, add=True)
                    mc.UVPin()
                    if not local_space:
                        mc.setAttr(joint_name+".inheritsTransform", 0)
            else:
                for i in range(number_of_joints):
                    mc.select(ribbon_group, replace=True)
                    ...
                    joint_name = mc.joint(position=position, radius=0.5, name=str(ribbon_object)+"_point"+str(i+1)+"_DEF")
                    if swap_uv:
                        mc.select(ribbon_object+".uv"+"["+str(v)+"]"+"["+str(u)+"]", replace=True)
                    else:
                        mc.select(ribbon_object+".uv"+"["+str(u)+"]"+"["+str(v)+"]", replace=True)
                    mc.select(joint_name, add=True)
                    mc.UVPin()
                    if not local_space:
                        mc.setAttr(joint_name+".inheritsTransform", 0)
